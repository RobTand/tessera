...
expert container to its rank on the group's shard plan (contract v26 moves
...
runtime produced for a fixed set of probe inputs. The inputs are this
repository's and the outputs are the kernel's; the table is generated by
`experiments/attest_activation_quantizer.py`, never hand-authored. Additive:
no format, cell, rung, route, launch, grade, KL entry, qualification, TP/EP
bound or served byte moves, and `lane_eligibility.schema` does not move. Only
`sm_121` and only `e2m1_group16_ue4m3_static` are attested; absence is a
refusal, not a default. See §4.5h.
...
disagrees with the sidecar; the FP8 route additionally refuses tensor
parallelism inside an expert outside the research-selected path, where the
NVFP4 route cuts it at load. When a checkpoint carries
`research_selected_moe`, only the stacks that block serves
(`ResearchSelectedMoeConfig.applies_to`: FP8/E4M3, BF16/BF16) take the
selected owner; an NVFP4 stack beside them takes its production builder, and
a block that names no stack it serves is refused at config parse.
...
### 4.5h What the contract says a serve EMITS, and why a name was not enough

A cell's `executes` says which launch runs. It does not say what that launch
computes. For the two quantized routes those are different questions, and the
fp4 one went unanswered until contract **v25** (#484).

`activation_contract` publishes `e2m1_group16_ue4m3_static`. Read it
carefully: E2M1 grid, groups of 16, a UE4M3 block scale, a static global. Every
one of those is a *shape*. None of them says how a value becomes a code --
which way an exact midpoint of the code lattice breaks, whether an over-range
block scale saturates to 448 or becomes an E4M3 NaN, whether the element path
carries an intermediate coarser than F32. The document carried no `round`,
`rcp`, `saturate`, `clamp` or `amax` anywhere. And the answer is not ours to
give: `nvfp4_route.apply` hands the artifact's `trellis_input_global_scale` to
vLLM's compiled `torch.ops._C.scaled_fp4_quant` (`native_ops.native_fp4_quant`),
so the rounding belongs to the pinned runtime.

A consumer that has to price the A side therefore had to re-implement the
quantizer and treat its own model as a runtime fact. RobTand/prismaquant#567 is
that bill arriving: with the global scale and every group's block scale
bit-identical on both sides, the emitted codes disagreed on 3.43 % of elements,
each by exactly one E2M1 step, and nothing in this document could say which
side was right.
...
- The **inputs** are the repository's. `serving/activation_attestation.py`
  ...
  refuses a published row whose input is not the one the spec builds. An edited
  input would make the row an assertion again.
- The **outputs** came from the operator and nowhere else, through
  `experiments/attest_activation_quantizer.py`, which the block names so a
  reader knows what to re-run. That script asks
  `nvfp4_route.blocked_scales` for the plane permutation rather than restating
  the hardware's swizzle, and proves the inverse by re-swizzling what it
  recovered.
- Every check the validator makes is *"a nearest encoding, or one of two tied
  nearest"* -- never *"this encoding"*. Which side of a tie the kernel takes is
  precisely the fact being published, so a checker that decided it would delete
  the reason the table exists. `tests/test_activation_attestation.py`
  `test_either_side_of_a_tie_is_accepted` holds that line.
- **Absence is not a default.** A platform or a contract with no entry
  publishes no attestation at all, and a consumer gate must refuse the route
  rather than fall back to its own model of the arithmetic.

**What sm_121 answered**, in the image `versions.default_serve_image` pins,
vLLM 0.28.0 / torch 2.13.0+cu130 on a GB10 at driver 595.84: round-to-nearest
with ties to the **even encoded index**, identical at all four probed used
scales (1.0, 1.5 at two global scales, 1.75 -- three of the four have inexact
F32 reciprocals): `0.25 -> 0`, `0.75 -> 1.0`, `1.25 -> 1.0`, `1.75 -> 2.0`,
`2.5 -> 2.0`, `3.5 -> 4.0`, `5.0 -> 4.0`. One BF16 ulp off a midpoint resolves
to the exact-nearest code every time, so the element path carries no
intermediate coarser than that. The block scale ties to zero at `2**-10` and
**saturates** to 448 at 512 rather than emitting NaN.

That is a **negative result** for the leading hypothesis about
prismaquant#567, and it is published as one: on every boundary this table
reaches, the kernel does what PrismaQuant's oracle models. The 3.43 %
divergence is not a tie-break, not a saturation convention and not a coarse
element intermediate.

**What it does not reach.** Every probe is exact on both sides by
construction, because a table whose inputs are inexact could not be checked
without the checker owning the runtime's own arithmetic -- the thing being
avoided. So the table attests behaviour at exactly representable global and
