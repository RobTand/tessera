...
**Disposition convention.** Every finding below carries one of `[FIXED <sha>]`,
`[#N]` or `[DISMISSED]`, and `tests/test_issue_refs.py` fails if any does not --
because a triage document that lists a real bug with no disposition is exactly
the artifact this project keeps mistaking for a tracker. The tracker is the
tracker; this file is the record of what was checked and what became of it.
Issue references are verified offline against `docs/issues-snapshot.json`
...
1. [#26] **§5 P0-3 CHANNEL refit has no `B>0` hold** (`scale_channel.py:245`). Its LUT
...
2. [#26] **§5 P0-4 `land_at_least` emits `inf`** (floor 65505 -> stored inf), and
...
3. [#26] **§5 P0-5 `shared_lut_global` admits subnormal moved bytes** (`fused.py:136`
...
4. [#26] **§5 P2-10d fused CHANNEL gap**: `prepare_tessera_module` feeds `scale_lut`
...
5. [FIXED 5445183] **§6 P0 TP shard start state dropped** -- and worse than the audit says.
...
6. [FIXED 5445183] **§6 P0 missing `rows % 8` / `cols % half` guards** in 5 of 6 kernel
...
7. [FIXED 7dd9a1a] **§2 P0-4/P0-5 partial trailing superblocks, reachable from `encode_linear`.**
...
8. [FIXED 7dd9a1a] **§2 P1-6 `Manifest.schedule` raises on a real E4M3 checkpoint.** The
...
9. [FIXED, #22] **Release can never reach a trailing partial superblock.** Found while
   ...
   so for a 640-column unit no release is ever placed in superblock 2. Encode
   and decode share the floor, so the round trip is symmetric -- verified, a
...
- [FIXED 7dd9a1a, wiring #23] §2 P0-1 `check_padding_zero` reads past the declared extent AND has zero
  callers -- so the padding-canonicality rule the wire declares is enforced
  nowhere.
- [FIXED 7dd9a1a] §2 P0-2 `positions // group_weights` truncates to 0 scale entries for
...
- [FIXED 7dd9a1a] §2 P0-3 partial-superblock terminal counts are not intersected with the
  descriptor's prefix sums; a boundary-violating terminal validates.
- [FIXED 7dd9a1a] §2 P1-7 `Reader.blob` has no `_MAX_BLOB` bound (the writer does).
- [FIXED 7dd9a1a] §2 P2-8 `write(1, 0)`-style zero-width writes silently drop the value.
- [FIXED 7dd9a1a] §2 P2-9 `quantizable_params` is only checked `> 0`; a wire value of 1e12
  validates and understates bpp by orders of magnitude.
- [FIXED 7dd9a1a] §2 P2-11 `q256_from_root(0) == 0` while `root_from_q256(0)` raises.
- [FIXED 249dc9a] §3 C1 `side_bytes` means two different things in `serialize` vs `parse`, so
...
- [FIXED 249dc9a] §3 C3 the pad-bit check ignores `bit_order` (LSB_FIRST is never constructed).
- [FIXED 249dc9a, dead branch #24] §3 C4 REFERENCE planes charge 0 bytes and nothing constructs one.
- [FIXED 249dc9a] §3 C5 `plane_rate` ignores padding; dead helper, zero callers.
- [FIXED 249dc9a] §3 G2b `unpack_uniform` slices trailing slack without checking it is zero.
- [FIXED 7dd9a1a] §3 G2c `build_terminal` with `plane_region=None` emits `sha256(zeros)` -- a
  look-valid digest of data never hashed. `calculator.terminal_rate` hits this
  every call.
- [FIXED 7dd9a1a] §3 G2d `max_released` is ignored whenever `spec` is present.
- [FIXED 249dc9a] §1 the whole section: `compose_half` takes a float `clip_exponent` and
...
- [#26] §5 P2-8 wrong-shaped refit metrics raise raw `RuntimeError`, not GrammarError.
- [#26] §5 P2-9 `land_at_least` overshoots the minimal fp16 word by up to 2 ulps
  (263/1500 cases).
- [#26] §5 P0-1/P0-2/P1-7 (S6b pack `floor` clips the group peak by up to 6.7%; the
