...
(contract_version **12** -- 11 on the branch, renumbered at merge because
another branch had already published an 11 for its `construction` block) gains
`native_extensions[].lane = {decoder, requires}`, and
