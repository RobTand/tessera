...
| arm | schedule | `out` | vs control | `hfit` | vs control | lever (same arm, CL off) |
|---|---|---|---|---|---|---|
| A control (served `h^1.0` x4) | `T R_h T R_h T R_h T R_h` | 0.05111 | 1.0000x | 0.04853 | 1.0000x | -- |
| `B-Jac` | `T R_h T R_h T R_h T R_H` | 0.04697 | 0.9191x | 0.04405 | 0.9077x | -- |
| `B-GS` | `T R_h T R_h T R_h T R_H(GS)` | 0.04523 | 0.8851x | 0.04265 | 0.8788x | -- |
| `C-Jac` | `T R_H T R_H T R_H T R_H` | 0.05043 | 0.9868x | 0.04759 | 0.9806x | -- |
| `C-GS` | `T R_H T R_H T R_H T R_H(GS)` | 0.04925 | 0.9635x | 0.04622 | 0.9524x | -- |
| `B-GS+CL` | `T R_h T R_h T R_h T R_H(GS,CL)` | 0.04108 | 0.8037x | 0.03718 | 0.7662x | **0.9081x** |
| `C-GS+CL` | `T R_H T R_H T R_H T R_H(GS,CL)` | 0.04722 | 0.9239x | 0.04399 | 0.9066x | **0.9589x** |
| A control, repeated last | `T R_h T R_h T R_h T R_h` | 0.05111 | 1.0000x | 0.04853 | 1.0000x | -- |

The lever is the last column: what the coupled landing is worth against the
*same* arm without it, which is the only comparison that isolates it. On the
trailing pass it is **0.9081x**; run on every pass it is **0.9589x** and the
arm it produces is worse in absolute terms (`C-GS+CL` 0.9239x against
`B-GS+CL`'s 0.8037x). Re-assigning early changes the codes the next trellis
pass sees, and that costs more than the assignment gains -- so `"trailing"` is
the arm to promote if any is, and running it everywhere is not a stronger
version of it.

Two controls make the table readable. The served default is encoded **first
and last** in the same process and lands on the same six numbers to every
digit printed, so nothing in between drifted. And every arm emits the same
`len(blob)` on every unit with `sink_vs_wire_bit_identical=True`, so all seven
are scored on bytes that weigh the same -- the landing changes which codes the
wire holds, never how many.

Receipt: `experiments/results/refit_trailing_pair_qwen_cl.json`
(`experiments/refit_trailing_pair.py --population qwen --coupled-landing`),
six dense Qwen3-0.6B units, `out` on held-out rows.
