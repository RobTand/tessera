...
**Not the default yet, and both mechanical gates are now closed.**
`DEFAULT_BODY` stays TCQ until the flip lands as a recipe change in
`wire_recipe`. The kernel lane **does** decode the body (see "Kernel lane"
below: bit-exact over both grids and all three planes, at the span-2
kernel's cost through L=14), and the encoder is no longer O(2^L) in
wall-clock: the fused Viterbi (see *The encoder, fused*) is 15× at L=12 and
26× at L=14 on a 2048×4096 tensor, bit-exact, profiled before and after.
PrismaQuant prices the shape-dependent recipe exactly (`def11bd`), and the
exporter records the recipe per rung (`dccbfd3`).
