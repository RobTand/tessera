...
1. **The EXL3 target is now measured, not fitted.** On the capture's held-out
   ...
   **1.176× behind on the weight leg, 1.070× under W4A4, and 1.017× in weight
   space.** The format gap is small; EXL3's remaining edge is Hessian shaping.
...
3. **Where Tessera-8 lives is 4–5 bpp, as a per-channel FP8 tile.** No block
   ...
   and 5.008 bpp it is at **weight-space parity with EXL3** (1.00×/1.03×),
   **1.13× behind under W?A8**, and its decoded tile is a stock per-channel
   FP8 tensor — the contract every FP8 tensor-core GEMM takes, on every
   vendor. That is the cross-platform opportunity, sized honestly: EXL3 with a
   per-platform decode kernel is still 13 % better in output space at those
   rates, and the lever that closes that (LDLQ, no wire change) is measured
   at 1.06–1.09× on a 4k-token Hessian.
...
5. **Against Gridbook:** Tessera-4 at 4.0 bpp beats FP8-CB K32 (4.0 bpw) by
   **1.09×**; Tessera-8 per-channel beats FP8-CB by **1.12×** at 4 and 5 bpw
   and **loses by 1.25×** at 6 bpw, where the E4M3 snapping of a 128-level
   codebook costs 1.21× and Gridbook's 4-D lattice VQ does not pay it.
   Below 4 bpp Gridbook's FP4-CB K24 (3.28 bpw) beats Tessera-4's sub-cap
   ladder by **1.09×** (interpolated) — the same builder-anchor problem.
...
In **weight space** the per-channel arms are at parity: R=4 0.0856 vs EXL3
0.0863 (0.99×), R=5 0.0450 vs 0.0439 (1.03×). EXL3's weight error is
*inflated* by its LDLQ (the reference's weak-Hessian control gave 0.0719 at
K=4); what it buys with that is the 1.13–1.19× in output space. That is the
lever Tessera has not used here (`compensate.py`; σ=1.0 measured 1.06–1.09×
...
**The per-16 plane is a better rate trade than the per-channel scale on
E4M3 too:** the minor-1 wire at R=5 (5.75 bpp, 0.0282) beats the per-channel
arm at R=6 (6.0 bpp, 0.0300). But a per-16 E4M3 scale plane is not an FP8
GEMM contract on any platform; per-channel is. (A per-32 **E8M0** plane would
be MXFP8, native on Blackwell only — unmeasured on E4M3; on E2M1 an E8M0-only
plane was 0.84×.)
...
- **Tessera-4 vs Gridbook FP8-CB K32 at 4.0 bpw:** Tessera **1.092× better**
  (out), 1.043× under A4. **vs Gridbook FP4-CB** at K24's 3.281 bpp (Tessera
...
- **EXL3@A8 target:** unreachable above ~5 bpp for any E4M3 tile (alphabet);
  at 4–5 bpp Tessera-8 per-channel is 1.13× behind under W?A8 and at parity
  in weight space, with LDLQ unspent.
- **Cross-platform:** the per-channel arm's decoded tile *is* a stock FP8
  weight. Serving it means either materialising E4M3 bytes (8 bpp resident —
  what Rob vetoed for the 4-bit lane) or a fused decode→FP8-GEMM kernel per
  platform. The analysis does not depend on which; the product does.
- **Gridbook:** Tessera wins at 4.0 (both families) and at 5.0 (FP8-CB) by
  9–12 %; Gridbook wins below 4.0 (FP4-CB) and at 6.0 (FP8-CB) by 9–25 %.
