...
| **Per-grid defaults** | `wire_recipe(grid, q256)`: E4M3 → window over CHANNEL, L=14, every rung; E2M1x2 → window over LUT16, L=12, below its cap (q256 < 896) and the coset trellis at it; E2M1 → the coset trellis. Flipped 2026-09-02 once the kernel lane decoded the body and the fused Viterbi encoded it. These are the RESEARCH defaults the recipe table records. A SERVED NVFP4 stack below the cap carries the span-2 TCQ spelling instead (`served_recipe` promotes it: the route decodes TCQ only), measurably worse than the WINDOW default it replaces — TCQ span-2 at 1.401×/1.357×/1.431× EXL3 at 2.5/3.0/3.5 bpp against window L=12 at 1.056×/1.061×/1.098× (§4) — necessary, stated, and stamped per rung in the contract's `attested_wire`. | E2M1 under the window body, and L=14 below the E2M1x2 cap, are measurements to run |
...
for an NVFP4 stack is span-2 TCQ at EVERY rung including below the cap
(`experiments/export_tessera_serving.py:served_recipe`, contract v32,
tessera#506 leg 2): the route has no WINDOW decode, so the served wire pays
the TCQ cost the frontier measured (§4: 1.36–1.43× vs 1.06–1.10× EXL3 at
2.5–3.5 bpp) rather than serving bytes nothing reads.
