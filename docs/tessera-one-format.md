...
| seam | state | plan |
|---|---|---|
| **Kernel lane** | decodes the span-2 TCQ wire over LUT16 (bit-exact, 0.0524 ms); a window decode is in flight (worker `ac41ec38`); no CHANNEL plane, no E4M3 GEMV | window decode lands → CHANNEL plane in the same GEMV → an FP8-tile GEMM path (decode to E4M3 bytes + per-row scale, feed the FP8 MMA) |
