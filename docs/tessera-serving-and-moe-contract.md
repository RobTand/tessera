...
## 12. The contract publishes what the plugin loads, not only what it runs (2026-09-03, #28)
...
**`native_extensions` is the table.** `contract_version` 5 → **6**, additive: a
v5 reader ignores a new top-level key and no value it already read has moved.
