...
repeated last, **recorded** byte- and tensor-identical (the encoder is
...
number of bytes, also recorded.  Neither is an ``assert``: a control that
disagrees is written into the receipt as ``!! DIFFER`` and as
``identical: false``, and the run continues, so **a reader has to look**
-- a green exit is not a passed control here.  ``--weights-only`` is the
matched control: same capture loaded, same scorer, the encode given no
Hessian.

**What is already on the tree, and what this adds (#80).**  Five reach
harnesses landed before this one -- ``reach_land_census.py``,
``reach_predictor_check.py``, ``dense4_reach_sweep.py``,
``dense_spread_census.py``, ``dense_spread_fix.py`` -- and
``reach_predictor_check.py`` in particular scores arms **already on disk** at
``/mnt/shared/tessera-runs/reach/reach_e4m3_*.json`` without re-encoding
anything.  Read that first: a question it can answer for free should not be
paid for here.  What this adds and none of them has is the combination of the
**BF16** grid, a full ``H_eval`` out-space score rather than a weight-space or
diagonal one, a production encode through ``ActivationSource``, and the
per-row SSE of every arm kept on disk.  The roster is also 56 dense units and
not four.  #80 is where the overlap is tracked; anyone scheduling this run
should say there which of the five it is not duplicating.
...
def load_dense(a):
    ...
    # The three fields above prove the two files are one *capture*.  They do
    # not prove the second one is the held-out half: hand ``--eval-h`` the fit
    # Hessian itself and every one of them matches, because it matches itself.
    # ``out`` would then be an in-sample score under a name that says held-out,
    # which is the failure this whole harness exists to avoid.
    if ev_prov.get("hessian_role") != "held-out":
        raise SystemExit(
            f"--eval-h {a.eval_h} does not declare hessian_role='held-out' "
            f"(got {ev_prov.get('hessian_role')!r}). The split fields match a "
            "file against itself, so the marker is the only thing separating "
            "the held-out half from the fit half; re-run capture_h_full.py "
            "with --eval-h-out to write one that carries it.")
    xs = {}
    if a.eval_x:
    ...
    def gen():
        for name in units:
            ...
            # The plane is the wire's, read per rung, so the loader hands out
            # a function of it rather than a fixed kwargs dict: the plane
            # decides what ``objective_for`` prices, and ``export_checkpoint``
            # reads it from ``wire_recipe`` for exactly that reason.  A literal
            # here is identical on every BF16 rung today and silently
            # mispriced the day one of them differs.
            def kw_for(plane, _n=name, _cols=w.shape[1], _act=act):
                return ({} if _act is None else
                        _act.for_unit(_n + ".weight", _cols, device="cuda",
                                      scale_plane=plane))
            he = ev_h[name].cuda().float()
            x = xs[name].cuda().float() if name in xs else None
            yield name, w, H.diagonal().contiguous(), he, kw_for, x
            del w, H, he, x
        ...
        for layer, proj in pairs:
            ...
            def kw_for(plane, _n=name, _cols=w.shape[1], _act=act):
                return ({} if _act is None else
                        _act.for_unit(_n + ".weight", _cols, device="cuda",
                                      scale_plane=plane))
            yield f"L{layer}.{proj}", w, H.diagonal().contiguous(), he, kw_for, x_ev
            del w, H, he, x_fit, x_ev
            torch.cuda.empty_cache()
    return meta, gen
...
def main() -> int:
    ...
    for name, w, h_fit_diag, h_eval, kw_for, x in gen():
        y = ny = None
        if x is not None:
            y = x @ w.T
            ny = float(y.norm())
