...
def main() -> None:
    ...
    if a.eval_h_out:
        # Same provenance block, deliberately: the three identity fields name
        # the FIT rows this matrix is disjoint from, and a scorer has to be
        # able to prove the two halves are one split -- which is exactly what
        # makes the payload dangerous.  It then carries ``export``'s whole
        # ``HESSIAN_IDENTITY`` and the shape ``ActivationSource.from_capture``
        # accepts, so a held-out matrix could be handed to an *encode* and the
        # identity comparison would say it was the fit capture.  ``role`` is
        # prose and no guard reads prose, so the marker a guard reads is
        # ``hessian_role``; ``from_capture`` refuses on it.
        Path(a.eval_h_out).parent.mkdir(parents=True, exist_ok=True)
