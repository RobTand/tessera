...
# up is not.
set -uo pipefail
# The checkout this script is committed in, so a driver run from a clone or a
# fresh worktree drives that one.  A hardcoded worker scratch path passes
# tests/test_experiment_import_roots.py only because it happens not to contain
# the substring the test looks for -- the #58 defect in the one shape the test
# cannot see.  Override with REPO=... for a deliberately foreign tree.
