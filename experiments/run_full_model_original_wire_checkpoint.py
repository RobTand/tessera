"""PB-owned full-census original-byte export in the canonical stock image."""
import json
import os
...
cmd = ['docker', 'run', '--gpus', 'all', '--ipc', 'host', '--cidfile', str(cidfile),
       '--name', 'tessera-full-original-export-' + uuid.uuid4().hex, '--network', 'none',
       ...
       '--evidence-dir', str(ROOT / 'plugin-install'), '--launcher-image-id', inspected['Id'],
       '--launcher-image-inspect', str(image_path), '--', 'python3',
       '/control/experiments/full_model_original_wire_checkpoint.py']
