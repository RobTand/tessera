...
PROMOTIONAL_POP_ROOT=/mnt/shared/tessera-runs/ts113-sparklina-population-aa6-r3
PROMOTIONAL_LOCAL_ROOT=/home/rob/tessera-runs/ts113-sparklina-aa6-r3
...
PREFIX=qwen_ts113_sparklina_aa6_r3
MODE=${1:-campaign}
DECODE_STRIDE=16
...
if [ "$MODE" = preflight-stage ]; then
  if prepare_stage path-expansion ts113-aa6-r3-preflight; then
...
if prepare_stage teacher ts113-aa6-r3-teacher; then
  if ! env \
      TESSERA_KL_IMAGE="$IMAGE" TESSERA_KL_EAGER=0 TESSERA_KL_REGIME=decode \
      TESSERA_KL_NAME=ts113-aa6-r3-teacher TESSERA_KL_LOGDIR="$TEACHER_DIR" \
...
run_arm() {
  ...
  local dir=$POP_ROOT/stages/$stage name=ts113-aa6-r3-$stage
