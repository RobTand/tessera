...
# ``torch._scaled_mm``.  (``config.py`` declared only ``serve_mode`` into
# the compile-cache identity at this script's merge base; #91 has since
# added the traced dispatch, so separate cache roots per arm are no
# longer what keeps two arms' builds apart.)  It declares
