...
def test_the_override_is_explicit_and_lands_in_the_manifest_record():
    ...
    ``--grid E2M1`` is the workflow that needs it: the route HOLDS the grid --
    the kernel admits arity 1 -- and the contract publishes no measured range
    for it, which is item 2's deliberate disagreement.  Exporting one anyway is
    a research artifact, and the reason to write it is the ``--stock-twin``,
    which vanilla vLLM serves with no plugin at all.

    ``--grid BF16`` was this test's example for about a day.  It stopped being
    one when the 16-bit route landed (#9) and gave that grid a decoder: the
    last assertion here is that BF16 now passes the gate, so the day a route is
    added the example moves rather than the test quietly asserting a stale
    refusal.
    """
